...
// ---------------- TTS (English only, with selectable voice) ----------------
const VOICE_STORAGE_KEY = "e504_voice_v1";

function getSavedVoiceURI(){
  try{ return localStorage.getItem(VOICE_STORAGE_KEY) || ""; }catch(e){ return ""; }
}
function saveVoiceURI(uri){
  try{ localStorage.setItem(VOICE_STORAGE_KEY, uri || ""); }catch(e){}
}
...
function populateVoiceSelect(){
  ...
  if(voices.length === 0){
    sel.innerHTML = `<option value="">صدای پیش‌فرض دستگاه</option>`;
    return;
  }
  sel.innerHTML = voices.map(v =>
    `<option value="${v.voiceURI}">${v.name} (${v.lang})${/network|online|wavenet|neural/i.test(v.name)?" 🌐 آنلاین":""}</option>`
  ).join("");
...
function speak(text, lang){
  ...
  if(savedURI){
    const match = getEnglishVoices().find(v => v.voiceURI === savedURI);
...
function navigate(name){
  ...
  if(name==="progress"){ renderProgressScreen(); populateVoiceSelect(); }
...
function renderQuizQuestion(){
  ...
  const speakBtnHtml = item.direction==="en2fa"
...
document.addEventListener("click", (e)=>{
  if(e.target && e.target.id === "btnRefreshVoices"){
    populateVoiceSelect();
    toast("لیست صداها بروزرسانی شد");
  }
});
...
// Support manifest shortcuts (?go=daily / dictionary / progress / lessons)
(function handleShortcut(){
  const params = new URLSearchParams(window.location.search);
  const go = params.get("go");
  if(go === "daily" || go === "dictionary" || go === "progress" || go === "lessons"){
    navigate(go);
  }
})();
